...
def score(in_ref, in_cmp, out):
    ...
    with fiona.open(in_cmp) as src:
        meta = copy(src.meta)
        meta['schema']['properties'] = {'polis': 'float:15.2','vertices': 'int','vertices_ref': 'int','diff_ver':'int'}
        with fiona.open(out, 'w', **src.meta) as sink: # create a new file 
            for rec in src:
                ...
                # problems
                vertices_ref=len(ref_polys[minscoreindex].exterior.coords)
    ...
    print("Number of misses: {}".format(len(shp_to_list(in_cmp)) - len(hits)))
...
def ComputeAverPolis(in_refdir, in_cmpdir):
    ...
    for cmpfile in glob.glob(join(in_cmpdir,"*.shp")):
        in_refpath= os.path.join(in_refdir,cmpfile)
        in_cmppath=join(in_cmpdir,cmpfile)
        filename,averagescore,aveverticdiff = score(in_refpath, in_cmppath,"test")
    ...
    split_name="PoLis"
    stats_filepath = os.path.join(in_cmpdir, "{}.csv".format(split_name))
